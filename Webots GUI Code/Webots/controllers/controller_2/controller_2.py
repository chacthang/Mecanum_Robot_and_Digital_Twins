...
class ExtU_digitaltwins_2_T(Structure):
    _fields_ = [
        ("Q", c_double * 3),
        ("Q_dot", c_double * 3),
        ("Qv", c_double * 3),
...
class ExtY_digitaltwins_2_T(Structure):
    _fields_ = [
        ("w1", c_double),
        ("w2", c_double),
        ("w3", c_double),
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.eight = ctypes.CDLL(r'D:\THANH\HK232\DoAnTN\VS\eight\digitaltwins_2\x64\Debug\digitaltwins_2.dll')
        self.eight.digitaltwins_2_initialize.restype = None
        self.eight.digitaltwins_2_initialize.argtypes = []
        self.eight.digitaltwins_2_step.restype = None
        self.eight.digitaltwins_2_step.argtypes = []
        self.eight.get_rtU.restype = POINTER(ExtU_digitaltwins_2_T)
        self.eight.get_rtU.argtypes = []
        self.eight.get_rtY.restype = POINTER(ExtY_digitaltwins_2_T)
        ...
        self.eight.digitaltwins_2_initialize()
        ...
        self.plotWidget.setXRange(1, 4, padding=0)
        self.plotWidget.setYRange(1, 4, padding=0)
        ...
        self.ref_x = np.sin(2*np.pi/15*t) + 2.5
        self.ref_y = np.sin(4*np.pi/15*t) + 3  
        self.ref_theta = 0
        ...
        #error
        self.filename = 'D:\THANH\HK232\DoAnTN\MATLAB\error\error_8_vir.csv'
        with open(self.filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Time", "Error_XP", "Error_YP", "Error_ThetaP", "Error_XV", "Error_YV", "Error_ThetaV"])
            
        self.filename1 = 'D:\THANH\HK232\DoAnTN\MATLAB\error\8_vir.csv'
    ...
    def run_trajectory(self):
        if self.radio_button_2.isChecked(): 
            ...
            """data0 = "0.00;0.00;0.00;0.00\r\n"
            if self.radio_button_2.isChecked():
                self.esp_socket.send(data0.encode("utf-8"))"""
            ...
            omega = (r/(4*0.6))*(-w1 + w2 + w3 - w4)
            
            """self.ref_x = np.sin(2*np.pi/15*(self.elapsed_time)) + 1.5
            self.ref_y = np.sin(4*np.pi/15*(self.elapsed_time)) + 1.5
            self.ref_theta = 0
            ...
            self.eight.digitaltwins_2_step()
