...
#include "digitaltwins_3.h"            /* Model's header file */
#include "rtwtypes.h"
...
unsigned __stdcall baseRateTaskScheduler(void *arg)
{
  ...
  noErr = (rtmGetErrorStatus(digitaltwins_3_M) == (NULL)) &&
    !rtmGetStopRequested(digitaltwins_3_M);
...
int main(int argc, char *argv[])
{
  ...
  /* Initialize model */
  digitaltwins_3_initialize();
  rtmSetErrorStatus(digitaltwins_3_M, 0);
  ...
  fflush(stdout);
  if (rtmGetErrorStatus(digitaltwins_3_M) != NULL) {
    fprintf(stderr, "\n**%s**\n", rtmGetErrorStatus(digitaltwins_3_M));
  }

  /* Disable rt_OneStep() here */

  /* Terminate model */
  digitaltwins_3_terminate();
  return 0;
}
