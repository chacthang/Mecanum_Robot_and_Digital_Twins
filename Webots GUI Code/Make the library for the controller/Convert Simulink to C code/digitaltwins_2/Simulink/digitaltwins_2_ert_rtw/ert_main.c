...
#include "digitaltwins_2.h"            /* Model's header file */
#include "rtwtypes.h"
...
unsigned __stdcall baseRateTaskScheduler(void *arg)
{
  ...
  noErr = (rtmGetErrorStatus(digitaltwins_2_M) == (NULL)) &&
    !rtmGetStopRequested(digitaltwins_2_M);
...
int main(int argc, char *argv[])
{
  ...
  /* Initialize model */
  digitaltwins_2_initialize();
  rtmSetErrorStatus(digitaltwins_2_M, 0);
  ...
  fflush(stdout);
  if (rtmGetErrorStatus(digitaltwins_2_M) != NULL) {
    fprintf(stderr, "\n**%s**\n", rtmGetErrorStatus(digitaltwins_2_M));
  }

  /* Disable rt_OneStep() here */

  /* Terminate model */
  digitaltwins_2_terminate();
  return 0;
}
